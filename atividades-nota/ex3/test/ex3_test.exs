defmodule Ex3Test do
  ...
  test "funcao map" do
    ...
    assert Ex3.map([1, 2, 3, 4, 5], fn x -> x * 2 end) == [2, 4, 6, 8, 10]
    assert Ex3.map(["foo", "bar", "baz"], fn s -> String.upcase(s) end) == ["FOO", "BAR", "BAZ"]
  ...
  test "fold_right" do
    ...
    assert Ex3.fold_right(["foo", "bar", "baz"], "", fn (s, st) -> s <> st end) == "foobarbaz"
  end
